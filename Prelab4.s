...
; Hardware: Contador binario de 4 bits utilizando dos (2) pushbuttons y cuatro (4) LEDs.
;
; Creado: 19 de febrero , 2022
...
reiniciar_tmr0 MACRO
    ...
    ;Prescaler = 256
    ;Fosc = 250kHz
    ;1 = 4*0.000004*(250-tmr0)*256
    ;24 = (256-tmr0)
    ;tmr0=178
    banksel PORTD
    movlw 178    ; Es lo que necesita para tener un impremento de 20ms
    movwf TMR0
...
push:   ; esta la indicación push
    movwf W_TEMP
    swapf STATUS, W
    movwf STATUS_TEMP
isr:  
    btfsc T0IF   ; Interrupcion del tmr0
    call int_t0
    
    btfsc RBIF  ; Interrupcion del on-change
    call int_iocb
...
int_iocb:  ;on-change
    ...
    incf PORT ; Se incrementa el PORTB si esta en 1
    btfss PORTB, DOWN
    decf PORT   ;Se decrementa el PORTB si esta en 0
    movf PORT, W
...
int_t0:
    reiniciar_tmr0 ; se utiliza el macro
    incf CONT  
    movf CONT,W
    sublw 50 ; Para llegar cada 1000ms
    btfss ZERO  ; si esta el bit 2 en cero
    goto RETURN_T0
    clrf CONT
    incf PORT1
    movf PORT1,W  ;para mostralo en el display
    call Tabla ; se traduce mediante la tabla
    movwf PORTD
    
    movf PORT1, W
    sublw 10   ; para que se encienda el display 1 de las decenas
    btfsc STATUS, 2
    call Incremento
...
Incremento:
    incf PORT3
    clrf PORT1
    movf PORT1,W ;Se mueve a la tabla
    call Tabla ; Se traduce
    movwf PORTD
    
    movf PORT3, W
    sublw 6  ; Se reinicia a llegar 60s
    btfsc STATUS, 2
    clrf PORT3
    return
...
main:
    ...
    call config_tmr0    ;Configaración del tmr0
    call config_int_enable ;Configuración de banderas
